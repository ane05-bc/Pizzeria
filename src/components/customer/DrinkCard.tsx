...
interface DrinkCardProps {
  drink: Drink;
  onAddDrinkToCart: (drinkId: string, quantity: number) => void;
}

export function DrinkCard({ drink, onAddDrinkToCart }: DrinkCardProps) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [quantity, setQuantity] = useState(1);

  const handleAddToCart = () => {
    onAddDrinkToCart(drink.id, quantity);
    ...
    <>
        ...
        <CardContent>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-card-foreground font-semibold">€{drink.price.toFixed(2)}</p>
            </div>
            <Button 
              onClick={() => setIsDialogOpen(true)}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl bg-card max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-card-foreground">{drink.name}</DialogTitle>
          </DialogHeader>
